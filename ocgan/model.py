...
class ocgan(object):
    ...
    def __init__(self, opt, dataloader=None):
        ...
        self.netde=Decoder(self.opt.isize,opt.nz,self.opt.nc,self.opt.ngf,opt.ngpu)
        ...
        self.fixed_input = torch.empty(size=(self.opt.batchsize, 1, self.opt.isize, self.opt.isize), dtype=torch.float32)
        self.real_label = 1 
        self.fake_label = 0
        self.u=None
        self.n=None
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(self.dataloader['test'], 0):
                ...
                time_o = time.time()
                test_ = self.an_scores[i* self.opt.batchsize: i*self.opt.batchsize + self.out_c.size(0)]
                
                if test_.shape[0] != self.out_c.shape[0]:
                    pass 
                else:
